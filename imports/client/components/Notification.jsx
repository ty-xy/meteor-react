...
class Notification extends Component {
    ...
    gotoLook = (e, _id, arg) => {
        e.preventDefault();
        const { nickname, plan, finish, help, num, toMembers } = arg;
        Meteor.call(
            'readLog',
            (_id),
            (err) => {
                if (!err) {
                    this.context.history.push({ pathname: '/manage/logging/detail', state: { nickname, plan, finish, _id, help, num, toMembers } });
                }
            },
        );
    }
    render() {
        ...
        return (
            <div className={classnames('e-notification', { 'e-notification-show': !isRead && !rejectRead })}>
